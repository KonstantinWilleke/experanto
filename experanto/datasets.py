...
class ChunkDataset(Dataset):
    ...
    def initialize_statistics(self) -> None:
        ...
        for device_name in self.device_names:
            ...
            if self.modality_config[device_name].transforms.get("normalization", False):
                ...
                self._statistics[device_name]["mean"] = means.reshape(1,
                                                                      -1)  # (n, 1) -> (1, n) for broadcasting in __get_item__
    ...
    def initialize_transforms(self):
        ...
        for device_name in self.device_names:
            if device_name == "screen":
                transform_list = [v for v in self.modality_config.screen.transforms.values() if
            ...
            if self.modality_config[device_name].transforms.get("normalization", False):
                transform_list.append(
                    v2.Normalize(self._statistics[device_name]["mean"].squeeze(),
                                 self._statistics[device_name]["std"].squeeze())
                )
            ...
            if "filters" in self.modality_config[modality]:
                ...
                for filter_name, filter_config in self.modality_config[modality]["filters"].items():
                    ...
                    if visualize:
                        ...
                        visualization_string = get_stats_for_valid_interval(valid_intervals_, self.start_time,
                                                                            self.end_time)
                        print(visualization_string)
                    if valid_intervals is None:
                        valid_intervals = valid_intervals_
                    else:
                        valid_intervals = find_intersection_between_two_interval_arrays(valid_intervals,
                                                                                        valid_intervals_)
    ...
    def get_screen_sample_mask_from_meta_conditions(self, satisfy_for_next: int,
                                                    valid_conditions_sum_of_product: List[dict],
                                                    filter_for_valid_intervals: bool = True) -> np.ndarray:
        ...
        filter_valid_intervals = self.get_valid_intervals_from_filters(
            visualize=False) if filter_for_valid_intervals else None
    ...
    def get_full_valid_sample_times(self, filter_for_valid_intervals: bool = True) -> Iterable:
        ...
        sample_mask_from_meta_conditions = self.get_screen_sample_mask_from_meta_conditions(chunk_size,
                                                                                            valid_conditions,
                                                                                            filter_for_valid_intervals)
